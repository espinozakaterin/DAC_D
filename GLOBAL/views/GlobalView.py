...
def get_accesos_global_security(request):
    ...
    appConn = connections['aws1_global']
    ...
    if user_id == '':
        ...
        try:
            ...
            with udcConn.cursor() as cursor:
                ...
                autorizaciones = cursor.fetchone()[0]
                
                # Módulos Red
                cursor.execute("SELECT COUNT(*) FROM modulos")
                modulos_red = cursor.fetchone()[0]

                # Actividad Reciente (Usuarios recién creados como ejemplo de actividad real)
                cursor.execute("SELECT Usuario, Nombre, PKUsuario FROM usuarios ORDER BY PKUsuario DESC LIMIT 5")
                ...
                for user in recent_users:
                    actividad.append({
                        ...
                        'desc': f"El usuario {user[0]} ({user[1]}) ha sido registrado en el sistema.",
                        'icono': 'user-plus',
                        'color': '#0ea5e9'
                    })
            ...
            # En caso de error, la actividad se mantiene vacía
            actividad = []
            pass
        ...
        context = {
            ...
            "modulos_red": modulos_red,
            "alertas": alertas,
            "actividad": actividad,
        }
        
        return render(request, 'global_main.html', context)
...
def global_acciones_usuario(request):
    ...
    if adminIT == 1:
            ...
            try:
                udcConn = connections['aws1_global']
                with udcConn.cursor() as cursor:
                    cursor.execute("SELECT PKModulo as PKmodulo, Nombre FROM modulos ORDER BY Nombre")
...
def asignar_modulo(request):
    ...
    if request.method == "POST":
        ...
        try:
            udcConn = connections['aws1_global']
            with udcConn.cursor() as cursor:
                cursor.callproc("mySP_Insert_Usuarios_Modulo", [modulo_id, usuario_id])
    ...
    if user_id == '':
        ...
        if adminIT == 1:
            ...
            try:
                udcConn = connections['aws1_global']
                with udcConn.cursor() as cursor:
                    # Fix for error (1305): Procedure does not exist. Using direct query.
                    cursor.execute("SELECT PKModulo as PKmodulo, Nombre FROM modulos ORDER BY Nombre")
                    column_names = [desc[0] for desc in cursor.description]
                    modulosData = [dict(zip(map(str, column_names), row)) for row in cursor.fetchall()]
...
def update_password_usuario(request):
    try:
        ...
        password_md5 = php_style_md5(password)
...
from dacd.utilsMain import php_style_md5
...
def insert_usuario_listado(request):
    ...
    password_md5 = php_style_md5(varPassword)
        ...
        if adminIT == 1:
            ...
            try:
                ...
                with udcConn.cursor() as cursor:
                    ...
                    modulosMenusGrupoData = []
                    for row in cursor.fetchall():
                        modulosMenusGrupoData.append(dict(zip(map(str, column_names), row)))
...
def get_menus_grupo_grupos_data(request):
    ...
    try:
        ...
        with udcConn.cursor() as cursor:
            ...
            for row in cursor.fetchall():
                row_dict = dict(zip(map(str, column_names), row))
                row_dict['IsBuiltIn'] = bool(row_dict['IsBuiltIn'])
                gruposData.append(row_dict)
...
def get_menus_grupo_menus_data(request):
    ...
    try:
        ...
        with udcConn.cursor() as cursor:
            ...
            for row in cursor.fetchall():
                row_dict = dict(zip(map(str, column_names), row))
                row_dict['Verificado'] = bool(row_dict['Verificado'])
                row_dict['Autorizado'] = bool(row_dict['Autorizado'])
                menusData.append(row_dict)
...
def global_config_tipos_solicitud_tokens(request):
    ...
    if adminIT != 1:
        return HttpResponseRedirect(reverse('global_main'))
    ...
    try:
        db_conn = connections['aws1_global']
        with db_conn.cursor() as cursor:
            cursor.execute("SELECT PKUsuario, Nombre FROM usuarios WHERE Estado = 1 ORDER BY Nombre ASC")
            cols_u = [desc[0] for desc in cursor.description]
            usuariosData = [dict(zip(map(str, cols_u), row)) for row in cursor.fetchall()]
            
            try:
                cursor.execute("SELECT PKsucursal, NombreSucursal FROM sucursales WHERE Estado = 1 ORDER BY NombreSucursal ASC")
                cols_s = [desc[0] for desc in cursor.description]
                sucursalesData = [dict(zip(map(str, cols_s), row)) for row in cursor.fetchall()]
            except:
                sucursalesData = []
        db_conn.close()

        # Preload Tipos Token from aws1_udc
        udc_conn = connections['aws1_udc']
        with udc_conn.cursor() as cursor:
            cursor.callproc("CTRL_TIPOS_TOKEN_GET", [1])
            cols_t = [desc[0] for desc in cursor.description]
            tiposTokenData = [dict(zip(map(str, cols_t), row)) for row in cursor.fetchall()]
        udc_conn.close()
    ...
    context = {
        ...
        "tipos_token_list": tiposTokenData,
        "error_db": error_db
    }
